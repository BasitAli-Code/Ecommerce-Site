    ...
    <>
      <div className="bg-[#E3E6F3] drop-shadow-lg px-6 md:px-18 flex justify-between items-center w-full h-20 fixed top-0 left-0 z-50">
        ...
        <div className="hidden md:flex">
          <Link
            to="/"
            className="text-2xl ml-14 transition-all duration-500 text-[#1a1a1a] hover:text-[#088178]"
          >
            Home
          </Link>
          <Link
        ...
        <div onClick={changeMenuState} className="block md:hidden">
          {isMenuSelected ? (
            <FaBars className="text-3xl text-[rgba(0,0,0,0.8)]" />
          ) : (
            <FaTimes className="text-3xl text-[rgba(0,0,0,0.8)]" />
          )}
        </div>
      </div>
